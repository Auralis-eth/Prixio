...
### Aha! Moment: Camera Launch Is a Product Feature
The WWDC2026 camera performance session reframed one thing that matters directly to Prixio: the scanner does not feel ready when the view appears; it feels ready when the first live preview frame appears. For a price-capture app, that matters because users are standing in an aisle, holding a phone at an awkward angle, trying to catch a shelf tag before the moment gets annoying.

The good news is that `CameraController` already follows two big rules from the session. It configures and starts `AVCaptureSession` on `Prixio.CameraSession` instead of blocking the main thread, and `CameraPreviewView` uses `AVCaptureVideoPreviewLayer`, which is the low-latency, low-overhead preview path when the app does not need per-frame access.

The next lesson is more specific: once the app targets the iOS 26 SDK, `AVCapturePhotoOutput` should be treated as non-critical launch work. The preview is the thing users need first; photo output readiness can be deferred after preview with the new Deferred Start API, paired with responsive capture so tapping the shutter immediately after launch still captures the moment instead of waiting for the output to finish stretching.
...
For Prixio, the practical wisdom is:
- keep `startRunning()` and configuration off the main thread
- keep using `AVCaptureVideoPreviewLayer` unless the app truly needs per-frame camera buffers
- defer non-preview outputs on iOS 26+
- enable responsive capture when deferring `AVCapturePhotoOutput`
- measure time-to-first-preview on real devices, including warm and thermally stressed conditions
