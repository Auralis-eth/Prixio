...
struct PriceParsingServiceDataSanitation {
    ...
    func typedPriceKindsSeparateRegularFromSaveAndDeposit() async throws {
        ...
        let observations = [
            OCRTextObservation(string: "Regular 6.49", confidence: 0.88),
            OCRTextObservation(string: "- SAVE", confidence: 0.84),
            OCRTextObservation(string: "$5.00 ea", confidence: 0.89),
            OCRTextObservation(string: "plus dep", confidence: 0.83),
            OCRTextObservation(string: "$0.10 deposit", confidence: 0.81)
        ]
    ...
    func standaloneShelfPriceIsTypedAsShelfCandidate() async throws {
        let scorer = PriceCandidateScorer()
        let observations = [
            OCRTextObservation(string: "Cadbury Chocolate Mini Eggs", confidence: 0.91),
            OCRTextObservation(string: "1799", confidence: 0.93)
        ]
        let candidates = scorer.scorePriceCandidates(
            scorer.extractPriceCandidates(from: observations),
            in: observations
