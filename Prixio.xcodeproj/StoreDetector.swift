actor PlacesAPIDetector: AppService {
    ...
    func detectStores(near location: CLLocation) async throws -> [StoreCandidate] {
        ...
        let radiusMeters = Int(Configuration.LocationServicesConfiguration.storeDetectionRadius)
        ...
        do {
            let results = try await service.searchNearbyPlaces(
                ...
                radius: radiusMeters,
                placeType: .grocery,
                maxResults: Configuration.LocationServicesConfiguration.maxStoreCandidates
            )
...
actor GPSDetector: AppService {
    ...
    func detectStores(near location: CLLocation) async throws -> [StoreCandidate] {
        ...
        do {
            nearbyStores = try await StoreDatabase().findNearbyStores(location: location, radius: 500)
        } catch {
            return []
        }
